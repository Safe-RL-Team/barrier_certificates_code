...
class SafeInvertedDoublePendulumEnv(InvertedDoublePendulumEnv, SafeEnv):
    episode_unsafe = False

    def __init__(self, threshold=1, task='hover', random_reset=False, violation_penalty=20):
    ...
    def step(self, a):
        ...
        if self.task == 'hover':  # set good reward for closer to boundary
            reward = (1.5 - np.abs(1.5 - y)) ** 2
        else:
            assert 0
    ...
    def reward_fn(self, states, actions, next_states):
        return -(next_states[..., 0] ** 2 + next_states[..., 1] ** 2 + next_states[..., 2] ** 2) - actions[
            ..., 0] ** 2 * 0.01
