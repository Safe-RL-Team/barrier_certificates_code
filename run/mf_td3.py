...
import safe.envs
from safe.envs.inverted_double_pendulum import *
from safe.envs.hopper import *
...
def main():
    ...
    register('SafeInvertedDoublePendulum-v2', entry_point=SafeInvertedDoublePendulumEnv, max_episode_steps=1000)
    ...
    hidden_sizes = [64, 64]
    qfn1 = MLPQFn([dim_state + dim_action, *hidden_sizes, 1]).to(device, dtype=float)
    qfn2 = MLPQFn([dim_state + dim_action, *hidden_sizes, 1]).to(device, dtype=float)
    policy = TanhGaussianMLPPolicy([dim_state, *hidden_sizes, dim_action * 2]).to(device, dtype=float)
