...
class SimpleRunner:
    ...
    @torch.no_grad()
    def run_w_record(self, policy, n_samples, dim_state, dim_action, filename = 'singlepend70000.gif',  buffer=None) -> List[Dict[str, float]]:
        n_steps = n_samples // self.n
        ep_stats = []
        ep_obs = []
        dur = 0.01
        width = 500
        height = 400
        states_array = np.zeros((dim_state, 1))
        actions_array = np.zeros((1, dim_action))
        ...
        for _ in range(n_steps):
            ...
            for i in range(self.n):
                next_state, reward, done, info = self.envs[i].step(actions[i].detach().cpu().numpy())
                obs = self.envs[i].render(mode='rgb_array', width=width, height=height)
                ep_obs.append(obs)
                timeout = bool(info.get('TimeLimit.truncated', False))
                done = bool(done) & ~timeout

                states_array = np.concatenate((states_array, next_state[:, np.newaxis]), axis=1)
                actions_array = np.concatenate((actions_array, actions), axis=0)
        ...
        if filename is not None:
            with imageio.get_writer(filename, mode='I', duration=dur) as writer:
                for obs_np in ep_obs:
                    writer.append_data(obs_np)
        

        return ep_stats, states_array[:,1:], actions_array[:,1:]
